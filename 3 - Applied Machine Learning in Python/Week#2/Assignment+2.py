...
def answer_one():
    ...
    result = []
    x_pre = np.linspace(0,10,100).reshape(-1,1)
    ...
    for degree in [1,3, 6, 9]:
        ...
        x_pre_poly = poly.fit_transform(x_pre)
                
        model = LinearRegression().fit(X_train_poly, y_train)
        y_predict = model.predict(x_pre_poly).reshape(1,-1)
        
        result.append(y_predict)
        
    result = np.reshape(result, (4, 100))
...
def answer_four():
    ...
    ling_model = LinearRegression().fit(X_train_poly, y_train)
    
    lasso_model = Lasso(alpha=0.01, max_iter=10000).fit(X_train_poly, y_train)
    
    LinearRegression_R2_test_score = r2_score(y_test , ling_model.predict(X_test_poly))
    Lasso_R2_test_score =  r2_score(y_test , lasso_model.predict(X_test_poly))
...
def answer_five():
    ...
    for idx in range(5):
        ...
        top_five.append(X_train2.columns[indices[idx]])
...
def answer_six():
    ...
    train_scores, valid_scores = validation_curve(clf, X_subset, y_subset,
                                            param_name='gamma',
    ...
    return (train_score_mean, valid_scores_mean)
#answer_six()
...
def answer_seven():
    ...
    #train_score_mean, valid_scores_mean = answer_six()
