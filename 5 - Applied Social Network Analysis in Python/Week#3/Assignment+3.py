...
def answer_four():
    ...
    return max(betweenness_centrality.keys(), key=lambda x: betweenness_centrality[x])
answer_four()
...
def answer_seven():
    ...
    G2_h, G2_a = nx.hits(G2) 
    
    return G2_h['realclearpolitics.com'], G2_a['realclearpolitics.com']
answer_seven()
...
def answer_eight():
    ...
    return sorted(G2_h.keys(), key = lambda x : G2_h[x], reverse = True)[:5]
answer_eight()
...
def answer_nine():
    ...
    return sorted(G2_a.keys(), key = lambda x : G2_a[x], reverse = True)[:5]
